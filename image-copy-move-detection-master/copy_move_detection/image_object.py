...
class ImageObject(object):
    ...
    def reconstruct(self):
        ...
        # Canvas for marking copy-move forgery
        groundtruth_image = np.zeros((self.image_height, self.image_width), dtype=np.uint8)
        lined_image = np.array(self.image_data.convert('RGB'))

        # --- Draw bounding boxes using OpenCV ---
        for key in sorted(self.offset_dictionary, key=lambda key: len(self.offset_dictionary[key]), reverse=True):
            if len(self.offset_dictionary[key]) < self.Nf * 2:
                break
            print(key, len(self.offset_dictionary[key]))
            for coord in self.offset_dictionary[key]:
                x, y = coord
                cv2.rectangle(lined_image, (x, y), (x + self.block_dimension, y + self.block_dimension), (0, 0, 255), 2)

                # For the grayscale mask
                for j in range(y, y + self.block_dimension):
                    for k in range(x, x + self.block_dimension):
                        if 0 <= j < self.image_height and 0 <= k < self.image_width:
                            groundtruth_image[j][k] = 255

        # --- Green edges logic remains unchanged ---
        for x in range(2, self.image_height - 2):
            for y in range(2, self.image_width - 2):
                if groundtruth_image[x, y] == 255 and (
                    groundtruth_image[x+1, y] == 0 or groundtruth_image[x-1, y] == 0 or
                    groundtruth_image[x, y+1] == 0 or groundtruth_image[x, y-1] == 0 or
                    groundtruth_image[x-1, y+1] == 0 or groundtruth_image[x+1, y+1] == 0 or
                    groundtruth_image[x-1, y-1] == 0 or groundtruth_image[x+1, y-1] == 0
                ):
                    lined_image[x, y, 1] = 255  # green edge line

        # Save both images
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        from imageio import imwrite

        mask_path = self.image_output_directory / f"{timestamp}_{self.image_name}"
        boxed_path = self.image_output_directory / f"{timestamp}_lined_{self.image_name}"

        imwrite(mask_path, groundtruth_image)
        imwrite(boxed_path, cv2.cvtColor(lined_image, cv2.COLOR_BGR2RGB))
